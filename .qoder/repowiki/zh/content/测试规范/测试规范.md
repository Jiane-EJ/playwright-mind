...
## 简介
本文件面向 HI-TEST 项目，系统化梳理登录页面测试与基础操作测试的规范、测试用例设计方法、测试报告生成与分析、自动化流水线实施策略以及调试与故障分析方法论。项目采用 Playwright 与 Midscene.js 结合的 AI 驱动测试框架，支持结构化任务驱动的验收测试与滑块验证码的自动/人工处理。**新增**跨平台通用配置支持和断言优先级策略，确保在多平台环境下的一致性和可靠性。
...
E --> I["src/stage2/task-runner.ts<br/>断言优先级策略实现"]
A --> J[".tasks/stage2登录滑块人工兜底实现_2026-03-12.md<br/>滑块处理模式说明"]
A --> K[".tasks/AI自主代理验收系统开发改造方案_2026-03-11.md<br/>推荐实践与断言策略"]
```
...
- **新增**断言优先级策略
  - src/stage2/task-runner.ts 实现了 Playwright 硬检测优先的断言策略，AI 操作作为兜底方案，确保在多平台环境下的稳定性和可靠性。

章节来源
- [README.md](file://README.md#L74-L162)
- [playwright.config.ts](file://playwright.config.ts#L22-L95)
- [tests/fixture/fixture.ts](file://tests/fixture/fixture.ts#L1-L100)
- [tests/generated/stage2-acceptance-runner.spec.ts](file://tests/generated/stage2-acceptance-runner.spec.ts#L1-L39)
- [src/stage2/types.ts](file://src/stage2/types.ts#L1-L180)
- [src/stage2/task-runner.ts](file://src/stage2/task-runner.ts#L1314-L1576)
- [specs/login-e2e.md](file://specs/login-e2e.md#L1-L152)
- [.tasks/stage2登录滑块人工兜底实现_2026-03-12.md](file://.tasks/stage2登录滑块人工兜底实现_2026-03-12.md#L1-L34)
...
TR["src/stage2/task-runner.ts<br/>断言优先级策略实现"]
end
subgraph "任务层"
...
- [src/stage2/types.ts](file://src/stage2/types.ts#L1-L180)
- [README.md](file://README.md#L140-L162)
...
#### 推荐实践与断言优先级
基于 AI 自主代理验收系统的开发经验，推荐采用以下实践：
- **断言优先级**：Playwright 硬检测优先（`getByRole/getByLabel/getByTestId` + 自动重试断言）
- **AI 查询策略**：语义复杂场景优先使用 `aiQuery + 代码断言`，减少 `aiAssert` 幻觉风险
- **AI 操作兜底**：AI 操作作为兜底，不建议所有步骤都直接依赖自由文本 AI 操作
- **关键断言原则**：官方明确建议，为降低 AI 幻觉风险，关键结果优先使用：
  - `aiQuery` 提取结构化信息
  - Playwright/TypeScript 代码做硬断言
  - `aiAssert` 作为补充性可读断言

#### 断言策略实现
src/stage2/task-runner.ts 实现了具体的断言策略：
- **Toast 断言**：先用 Playwright 硬检测，未检测到时使用 AI 兜底
- **表格行断言**：先用 Playwright 硬检测，未检测到时使用 AI 兜底
- **表格单元格断言**：使用 AI Query + 代码断言的组合策略
- **通用断言**：未知断言类型使用 aiQuery 结构化验证作为保底

```mermaid
flowchart TD
Start(["断言执行"]) --> Type{"断言类型"}
Type --> |toast| Toast["Toast断言"]
Type --> |table-row-exists| TableRow["表格行断言"]
Type --> |table-cell| TableCell["表格单元格断言"]
Type --> |custom| Custom["自定义断言"]
Toast --> PWToast["Playwright硬检测"]
PWToast --> |成功| Success1["断言通过"]
PWToast --> |失败| AIToast["AI断言兜底"]
AIToast --> Success1
TableRow --> PWRow["Playwright硬检测"]
PWRow --> |成功| Success2["断言通过"]
PWRow --> |失败| AIRow["AI断言兜底"]
AIRow --> Success2
TableCell --> AIQuery["AI Query提取结构化信息"]
AIQuery --> CodeAssert["代码断言验证"]
CodeAssert --> Success3["断言通过"]
Custom --> AICommon["AI通用断言"]
AICommon --> Success4["断言通过"]
```

图表来源
- [src/stage2/task-runner.ts](file://src/stage2/task-runner.ts#L1314-L1576)
- [README.md](file://README.md#L107-L111)
- [.tasks/AI自主代理验收系统开发改造方案_2026-03-11.md](file://.tasks/AI自主代理验收系统开发改造方案_2026-03-11.md#L78-L91)
...
- 组件耦合
  ...
  - **新增**src/stage2/task-runner.ts 实现断言优先级策略，为所有断言提供统一的执行逻辑。
- 外部依赖
...
RunnerSpec --> TaskRunner["src/stage2/task-runner.ts<br/>断言优先级策略"]
...
- **新增**断言策略性能优化
  - 断言优先级策略通过缩短超时时间（如将 timeoutMs 分配给 Playwright 检测的前半部分）提升执行效率。
  - 重试机制合理配置，避免过度重试影响整体性能。
...
- 常见问题定位
  ...
  - **新增**跨平台适配问题：检查 uiProfile 中的 selectors 配置是否正确，必要时添加新的选择器优先级。
- 调试方法
  - 使用 --headed 模式运行，观察交互过程。
  - 查看 t_runtime 目录下的截图、视频与 trace，结合 Midscene 报告定位问题。
  - 对失败步骤打印详细信息（失败步骤名、消息、截图路径），便于快速定位。
  - **新增**断言策略调试：关注断言降级日志，确认 Playwright 检测失败后的 AI 兜底是否正常工作。
- 故障分析流程
  - 复现 → 截图/trace → 选择器/环境变量核对 → 模式切换（滑块）→ **新增**跨平台配置调整 → 断言策略优化 → 修复 → 回归验证。
...
## 结论
本测试规范文档基于 HI-TEST 项目的现有实现，明确了登录页面测试与基础操作测试的流程、验证要点与最佳实践，给出了测试用例设计、报告生成与分析、自动化流水线实施与调试故障分析的方法论。**新增**的跨平台通用配置和断言优先级策略进一步提升了项目的可扩展性和可靠性。通过统一的运行时目录、夹具与任务模型，以及 Playwright 硬检测优先的断言策略，项目实现了可重复、可观测、可扩展的 AI 驱动测试体系。
...
- **新增**跨平台配置字段
  ...
  - uiProfile.toastSelectors：消息提示选择器优先级列表
  - uiProfile.dialogSelectors：弹窗选择器优先级列表
  - assertions[].matchMode：行匹配模式（exact/contains）
  - cleanup.rowMatchMode：清理时行匹配模式（建议 exact）
  - cleanup.verifyAfterCleanup：删除后校验目标行消失（建议 true）
